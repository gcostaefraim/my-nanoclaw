...
Add your token to `.env` using the `CONTAINER_` prefix — NanoClaw injects it automatically:

```bash
# .env
CONTAINER_TODOIST_API_TOKEN=your_token_here
```

```bash
td auth status   # verify authentication inside the container
```
